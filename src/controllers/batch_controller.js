...
export const deleteOrphanBatchController = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: "Error al eliminar el lote huérfano.", error: errorMessage });
    }
};
...
export const clearDocumentsController = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ 
            ...
            docType, fromDoc, toDoc, clearedCount
        });
    } catch (error) {
        const errorMessage = error.message || String(error);
        logger.error(`Fallo al limpiar documentos: ${errorMessage}`);
        res.status(500).json({ message: "Error al limpiar documentos.", error: errorMessage });
    }
};
